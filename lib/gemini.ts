...
8. Demo 互動體驗設計：根據顧客的年齡、職業、生活習慣，推薦 1-2 個讓對方突然意識到「我每天都在被 UIUX 設計師設計卻不知道」的互動問題，目標是製造頓悟感。
   ...
   - 接話原則：不管對方回答什麼（包括「不知道」「沒想過」「因為好看」），業務都要能用一句話接住，並說明這是設計師刻意這樣做的結果，不是隨便決定的
   - 接話範例格式：「你說的沒錯，這其實是設計師為了[讓你做某個行為]而故意這樣設計的——[用1句話解釋設計意圖]」
   - 每個互動問題最後必須有一句「頓悟收尾句」，讓對方意識到自己每天都在被設計卻不知道，範例：「所以你看，你每天滑手機、用 app、去便利商店，其實都在被設計師設計，只是你以前不知道而已」
